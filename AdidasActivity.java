...
public class AdidasActivity extends AppCompatActivity {
    private ActivityAdidasBinding binding;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        EdgeToEdge.enable(this);
        binding = ActivityAdidasBinding.inflate(getLayoutInflater());
    ...
    private void initAdidas() {
        ...
        myref.orderByChild("rating").endAt(1).addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (snapshot.exists()) {
                    ...
                    if (!itemDomains.isEmpty()) {
                        binding.recyl.setLayoutManager(new GridLayoutManager(AdidasActivity.this, 2));
